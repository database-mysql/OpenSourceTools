    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender instanceof Player) {
            if (args.length == 1) {
                ...
                TextComponent AccentMessage = new TextComponent(multi.ColorMessage("&7[&a&l同意&7]"));
                AccentMessage.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(multi.ColorMessage("&a接受" + PlayerName + "的传送请求"))));
                AccentMessage.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/tpaaccept "+ PlayerName));
                TextComponent RefuseMessage = new TextComponent(multi.ColorMessage("&7[&d&l拒绝&7]"));
                RefuseMessage.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(multi.ColorMessage("&a拒绝" + PlayerName + "的传送请求"))));
